...
#1-2. 데이터 노이즈 추가
# 정규분포 np.random.normal(0,0.5) = 평균이 0이고, 표준편차가 0.3인 랜덤값을 만들어서 x_train에 점을 찍어줘라
# 표준편차가 커질수록 높이는 낮고 양옆 길이가 길어지는 정규분포가 생성
x_train_noise = x_train + np.random.normal(0,0.3,size=x_train.shape)
...
model.fit(x_train_noise, x_train, epochs=30)
