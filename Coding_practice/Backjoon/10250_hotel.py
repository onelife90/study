...
#1. 테스트 데이터 입력받기
#2. for 문 안에 split()로 h,w,n 입력받기
#3. floor = 도착한 순서 % 층 (층대로 선착순이니까 나머지가 필요)
#4. room = 도착한 순서 // 층+1 (컬럼으로 생각해서 몫이 다 차고, +1을 해줘야 방 번호)
#5. 나머지가 없을 경우
...
'''   방 번호부터 만드려고 했으나 몇번째 손님이 몇층, 몇호인지가 중요
