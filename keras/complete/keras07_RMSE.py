...
# 회귀 분석 평가 시 가장 많이 쓰는 지표 중 하나. 인공지능 대회나 캐글은 RMSE로 평가하는 경우가 많다
# MSE -> RMSE
# (실제값-예측값)^2의 합/n 한 후 루트 씌우기 => 루트 씌우기 때문에 값이 작을수록 좋음
...
def RMSE(y_test, y_predict): 
    return np.sqrt(mean_squared_error(y_test, y_predict)) 
            # 반환 = 출력값
            # sqrt = 루트
            # mse=(실제값-예측값)^2의 합/n 한 후 루트. y_test(실제값), y_predict(예측값)가 입력값이 됨
