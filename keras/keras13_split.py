...
# 2) validation_data=(x_val, y_val)에 x_val, y_val를 삭제했기 때문에 쓸수가없음. 다른 파라미터로 대체
# 3) 그럼 어떤 파라미터? 구글링에 소질이 없나봄.. 꿀팁! model.fit validation_까지만 치면 연관검색어에 model.fit validation_data 바로 밑에 split이 나옴
# 4) model.fit validation_split를 쓰면 train 비율 X split 비율을 하면 val의 비율과 train의 비율이 나옴
